...
def custom_cross_val_predict(X, y, cv, label1_weight, sample_weight, cfg)\
    ...
    for train_idx, valid_idx, test_idx in cv.split(X, y):
        ...
        if cfg.model_name == 'xgb':
            ...
            model_onnx = onnxmltools.convert_xgboost(model[-1], 
                    initial_types = [('input', FloatTensorType([None, 186]))])
            onnxmltools.utils.save_model(model_onnx, save_name)
            ...
            if isinstance(X_te.astype(np.float32), pd.DataFrame):
                sess_input = scaler.fit_transform(
                                X_te.astype(np.float32).to_numpy()
                             )
            else:
                sess_input = scaler.fit_transform(
        ...
        # ONNX 모델 결과 다른지 확인
        pred_model = model[-1].predict(sess_input)
        pred_model_prob = model[-1].predict_proba(sess_input)
        print(
    ...
    pred_model_prob[pred_model == 1]
    [pred_onnx[1][i] for i in range(len(pred_onnx[0])) if pred_onnx[0][i] == 1]
